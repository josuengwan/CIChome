...
//업로드 파일 있는지 검사
$filename = null;
if(strlen($_FILES['file']['name'])==0){		 //파일 없음

	$query="insert into $board (id,name,num,date,text,count,title,file) values ('$id','$name','$num','$time','$text','0','$title','none')";
	...
	if($ext != 'php' && $ext != 'inc' && $ext != 'html' && $ext != 'htm' && $ext != 'phtml' && $ext != 'php3' ){  //확장자 구분
		...
		$query="insert into $board (id,name,num,date,text,count,title,file) values ('$id','$name','$num','$time','$text','0','$title','$filename')";
	}else{	//확장자 없음
