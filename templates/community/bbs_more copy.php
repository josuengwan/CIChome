...
<?
		...
		<tr>";
			if($id=="Admin" || $select == 2 && $id == $row[2]  && $id != guest || $select == 5 && $id == $row[2]) 
			{
				...
				<td><a href='./bbs?select=$select' class='button'>목록</a></td><td>&nbsp;</td><td>
				<a href='./bbs_edit?select=$select&num=$num' class='button'>수정</a><td>&nbsp;</td><td>
				<a href='./bbs_delete?select=$select&num=$num' class='button'>삭제</a>";
			}else{
				echo "<td width=640></td><td>&nbsp;<a href='./bbs?select=$select' class='button'>목록</a>";
			}
			echo"
			</td>
		</tr>
